...
def fetch_logs_http_fast(session, rpc_url, contract_addr, topic0, from_block, to_block, chunk_size=2000):
    contract_addr = Web3.to_checksum_address(contract_addr)
    all_logs = []
    curr = from_block
    payload_id = 1

    print(f"[*] Scanning {contract_addr[:10]}... logs from block {curr} to {to_block}...", flush=True)
...
def main():
    print("=" * 80)
    print("SCANNING CELO MAINNET LOGS & TOKEN REGISTRY ENRICHMENT")
    print("=" * 80)
    ...
    print(f"[+] Contract Creation Block : {START_BLOCK}\n")
    ...
    vortex_price_logs = fetch_logs_http_fast(session, CELO_RPC_URL, VORTEX_ADDRESS, VORTEX_PRICE_UPDATED_TOPIC0, START_BLOCK, latest_block)
    print(f"    -> Retrieved {len(vortex_price_logs)} Vortex PriceUpdated events.", flush=True)
    ...
    vortex_traded_logs = fetch_logs_http_fast(session, CELO_RPC_URL, VORTEX_ADDRESS, VORTEX_TOKEN_TRADED_TOPIC0, START_BLOCK, latest_block)
    print(f"    -> Retrieved {len(vortex_traded_logs)} Vortex TokenTraded events.", flush=True)
    ...
    ctrl_traded_logs = fetch_logs_http_fast(session, CELO_RPC_URL, CONTROLLER_ADDRESS, CONTROLLER_TOKENS_TRADED_TOPIC0, START_BLOCK, latest_block)
    print(f"    -> Retrieved {len(ctrl_traded_logs)} Controller TokensTraded events.", flush=True)
