...
<head>
    ...
    <title>Vortex Ethereum - Trade Analytics Dashboard</title>
    ...
    <style>
        ...
        * {{
        ...
        .btn-view-trades {{
            background: linear-gradient(135deg, rgba(99, 102, 241, 0.35), rgba(16, 185, 129, 0.35));
        ...
        /* Modal Overlay Styling - Guaranteed Top Layer */
        #trade-modal-overlay {{
...
<body>
    ...
    <div class="navbar">
        <div class="brand">
            ...
            <div class="brand-title">Carbon Vortex Tracker</div>
        </div>
    ...
    <div class="container">
        ...
        <div class="meta-banner">
            <div class="meta-item">
                ...
                <span class="meta-val">0xD053Dcd7037AF7204cecE544Ea9F227824d79801</span>
            </div>
            <div class="meta-item">
                <span class="meta-label">Start Block</span>
                <span class="meta-val">20,469,390</span>
            </div>
            <div class="meta-item">
                <span class="meta-label">Topic0 Event Filter</span>
                <span class="meta-val">0x16ddee9b3f1b2e6f797172fe2cd10a214e749294074e075e451f95aecd0b958c</span>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value" id="stat-events">703</div>
                <div class="stat-sub">⚡ Scanned via Etherscan V2 API</div>
            </div>
            <div class="stat-card">
                <div class="stat-title">Unique Tokens Traded</div>
                <div class="stat-value" id="stat-tokens">86</div>
                <div class="stat-sub">💎 Decoded from topic2</div>
            </div>
            <div class="stat-card">
                <div class="stat-title">Level 1 (Token → ETH)</div>
                <div class="stat-value" id="stat-l1">85 Tokens</div>
                <div class="stat-sub">Fee Tokens → ETH (target)</div>
            </div>
            <div class="stat-card">
                <div class="stat-title">Level 2 (ETH → BNT)</div>
                <div class="stat-value" id="stat-l2">414 Trades</div>
                <div class="stat-sub">ETH → BNT (final target)</div>
            </div>
        </div>
        ...
        <div class="controls-bar">
            ...
            <div class="control-group">
                ...
                <div style="display: flex; gap: 6px; margin-left: 10px;">
                    <button class="btn-filter active" onclick="setFilter('all', this)">All Tokens (86)</button>
                    <button class="btn-filter" onclick="setFilter('l1', this)">Level 1 (Token → ETH)</button>
                    <button class="btn-filter" onclick="setFilter('l2', this)">Level 2 (ETH → BNT)</button>
                </div>
            </div>
        </div>
    ...
    <!-- Trade History Floating Modal Window Overlay -->
            ...
            <div class="modal-body">
                <div class="trade-detail-summary">
                    <div class="detail-item">
                        <span class="detail-label">Trade Level Rule</span>
                        <span class="detail-val" id="modal-level-val">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Total Volume ($ USD)</span>
                        <span class="detail-val" id="modal-usd-val" style="color: #10b981;">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Total Target Released</span>
                        <span class="detail-val" id="modal-total-amt">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Total Source Paid</span>
                        <span class="detail-val" id="modal-total-source" style="color: #818cf8;">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Total Trades Executed</span>
                        <span class="detail-val" id="modal-trade-cnt" style="color: #fbbf24;">-</span>
                    </div>
                </div>
                ...
                <div class="table-responsive" style="border-radius: 14px; border: 1px solid var(--card-border);">
                    <table>
                        ...
                        <tbody id="modal-trade-tbody">
                            <!-- Dynamic Trade Detail Rows -->
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    <script>
        ...
        function renderTable() {{
            ...
            let filtered = rawData.filter(item => {{
                const matchesSearch = item.symbol.toLowerCase().includes(searchQuery) || 
            ...
            filtered.forEach((item, index) => {{
                ...
                const etherscanUrl = 'https://etherscan.io/address/' + item.address;
                
                const formattedTargetAmount = item.total_amount < 1000000 
                    ? item.total_amount.toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 4}})
                    : item.total_amount.toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 2}});

                const formattedSourcePaid = item.total_source_amount < 1000000
                    ? item.total_source_amount.toLocaleString(undefined, {{minimumFractionDigits: 4, maximumFractionDigits: 6}})
                    : item.total_source_amount.toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 2}});
                ...
                const levelText = item.level === 2 ? 'Level 2: ETH → BNT' : 'Level 1: ' + item.symbol + ' → ETH';

                tr.innerHTML = `
                    <td style="color: var(--text-muted); font-weight: 500;">${{index + 1}}</td>
                    <td>
                        <div class="token-cell">
                            <div class="token-avatar">${{symbolInitial}}</div>
                            <div>
                    ...
                    <td>
                        <span class="level-badge ${{levelBadgeClass}}">${{levelText}}</span>
                    </td>
                    <td>
                        <div class="address-cell">
                            <a href="${{etherscanUrl}}" target="_blank" class="link-etherscan">${{shortAddr}}</a>
                            <button class="btn-copy">Copy</button>
                        </div>
                    </td>
                    <td style="text-align: center;">
                        <button class="btn-view-trades">
                            📊 View ${{item.trade_count}} Trades
                        </button>
                    </td>
                    <td style="text-align: right;">
                        <div class="source-paid-val">${{formattedSourcePaid}} ${{item.source_symbol}}</div>
                    </td>
                    <td style="text-align: right;">
                        <div class="amount-val">${{formattedTargetAmount}} ${{item.symbol}}</div>
                    </td>
                    <td style="text-align: right;">
                        <div class="usd-val">${{formattedUsd}}</div>
                ...
                // Attach JS event listeners safely
                const copyBtn = tr.querySelector('.btn-copy');
                if (copyBtn) {{
                    copyBtn.addEventListener('click', (e) => {{
                        e.stopPropagation();
                        copyToClipboard(item.address, copyBtn);
                    }});
                }}
                ...
                tr.addEventListener('click', (e) => {{
                    if (e.target.tagName === 'A' || e.target.classList.contains('btn-copy')) return;
                    openTradeModal(item);
                }});
        ...
        /* 100% Fail-Proof Floating Modal Window Engine */
        ...
        function openTradeModal(item) {{
            console.log('Opening modal window for item:', item.symbol);
            ...
            setElText('modal-level-val', item.level === 2 ? 'Level 2 (ETH → BNT Final Burn)' : 'Level 1 (' + item.symbol + ' → ETH Target)');
            ...
            if (tbody) {{
                ...
                (item.trades || []).forEach(tr => {{
                    ...
                    const txShort = tr.tx_hash ? tr.tx_hash.substring(0, 10) + '...' + tr.tx_hash.substring(58) : 'N/A';
                    const txUrl = tr.tx_hash ? 'https://etherscan.io/tx/' + tr.tx_hash : '#';
                    const callerShort = tr.caller ? tr.caller.substring(0, 8) + '...' + tr.caller.substring(34) : 'N/A';
                    
                    const fmtTarget = tr.target_formatted.toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 4}});
                    const fmtSource = tr.source_formatted.toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 6}});
                    const fmtUsd = '$' + (tr.usd_value || 0).toLocaleString(undefined, {{minimumFractionDigits: 2, maximumFractionDigits: 2}});
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <a href="${{txUrl}}" target="_blank" class="link-etherscan" style="font-family: 'JetBrains Mono', monospace; font-size: 13px;">${{txShort}}</a>
                        </td>
                        <td style="font-family: 'JetBrains Mono', monospace; font-size: 13px; color: var(--text-muted);">${{callerShort}}</td>
                        <td style="text-align: right; font-family: 'JetBrains Mono', monospace; font-weight: 600; color: #10b981;">${{fmtTarget}} ${{item.symbol}}</td>
                        <td style="text-align: right; font-family: 'JetBrains Mono', monospace; font-weight: 600; color: #818cf8;">${{fmtSource}} ${{tr.source_symbol}}</td>
                        <td style="text-align: right; font-family: 'JetBrains Mono', monospace; font-weight: 600; color: #34d399;">${{fmtUsd}}</td>
                    `;
        ...
        const overlayEl = document.getElementById('trade-modal-overlay');
        if (overlayEl) {{
            overlayEl.addEventListener('click', (e) => {{
                if (e.target.id === 'trade-modal-overlay') {{
                    closeModal();
                }}
            }});
        }}
...
"""
    ...
    print(f"Generated dashboard HTML: {OUTPUT_HTML}")
