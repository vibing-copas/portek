...
COMPARISON_FILE = os.path.join("data", "controller_topic3_vs_token_registry_comparison.json")
...
def main():
    print("=" * 80)
    print("INSERTING MISSING CONTROLLER TOPIC3 TOKENS INTO TOKEN_REGISTRY (DATA/TRACKER.DB)")
    print("=" * 80)
    ...
    missing_tokens = comparison_data.get("only_in_topic3", [])
    ...
    print(f"[+] Found {len(missing_addresses)} missing Controller tokens to enrich & insert into DB.\n")
    ...
    if os.path.exists(LOGS_FILE):
        ...
        for log in all_logs:
            ...
            if len(topics) > 3 and topics[3]:
                clean_topic3 = topics[3].lower()
                if len(clean_topic3) >= 40:
                    addr = "0x" + clean_topic3[-40:]
    ...
    for idx, raw_addr in enumerate(missing_addresses, 1):
        ...
        print(f"[{idx:2d}/{len(missing_addresses)}] Fetching metadata for {checksum_addr}...")

        # Fetch symbol & decimals
        meta = fetch_token_metadata_rpc(w3, checksum_addr)
        print(f"     -> Symbol: {meta['symbol']:<12} | Decimals: {meta['decimals']}")
    ...
    print("\n" + "=" * 90)
    print("SUCCESSFULLY INSERTED 21 CONTROLLER TOPIC3 TOKENS INTO TOKEN_REGISTRY:")
    print("=" * 90)
    print(f"{'#':<3} | {'SYMBOL':<12} | {'DEC':<4} | {'EVENTS':<6} | {'FIRST BLK':<10} | {'LAST BLK':<10} | {'ADDRESS':<42}")
    print("-" * 90)
    for idx, item in enumerate(inserted_list, 1):
        print(f"{idx:<3} | {item['symbol']:<12} | {item['decimals']:<4} | {item['events']:<6} | {str(item['first_seen_block']):<10} | {str(item['last_seen_block']):<10} | {item['token_address']}")
    print("=" * 90)
